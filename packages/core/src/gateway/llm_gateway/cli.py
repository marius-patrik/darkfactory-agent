...
from llm_gateway.registry import ModelRegistry, ActiveRoleManager, ModelEntry, RegistryError, ROLE_NAMES
from llm_gateway.task_routing import TaskRouter, TaskRoutingError

app = typer.Typer(name="gateway", help="LLM Gateway CLI")
console = Console()
...
def model_select(
    role: str = typer.Argument(..., help="Role: general | coding | conversation | judge"),
    model_id: str = typer.Argument(...),
) -> None:
    ...
    if role not in ROLE_NAMES:
        console.print(f"[red]Role '{role}' is not supported[/red]")
        raise typer.Exit(1)
