...
is **copied + adapted** into `llm-gateway` as `llm_gateway`. The v3
...
1. **Re-namespace** the v3 `agents` package namespace →
   `llm_gateway` (package, imports, response-metadata key `llm_gateway`,
...
- **The embedding model** (§13 #5): no embedding model is registered yet, so
  the `embedding` role is hidden from public role and switcher surfaces (VS1
  ships general/coding/conversation only).
