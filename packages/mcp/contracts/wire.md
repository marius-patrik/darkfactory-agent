...
[`packages/mcp/proto/agent_os/v1/`](../../src/migrate/core/proto/agent_os/v1) is
...
> module under [`packages/mcp/proto/agent_os/v1/`](../../src/migrate/core/proto/agent_os/v1); this doc is its map.
>
...
```sh
(cd packages/migrate/core && bunx --bun @bufbuild/buf generate proto)
...
| **TS** | `packages/migrate/core/clients/shared-ts/src/gen` | `bufbuild/es` (protobuf-es v2) | `@bufbuild/protobuf`, `@connectrpc/connect` (`bun install`) |
...
`packages/migrate/core` is an internal implementation subtree, not an independently
...
There is no user-facing CLI or installer in `packages/migrate/core`.
