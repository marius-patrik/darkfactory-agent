...
app = typer.Typer(name="gateway", help="Agents LLM Gateway CLI")
console = Console()
...
def serve(
    ...
    import uvicorn
    from agents.packages.gateway.gateway.main import app
    uvicorn.run("gateway.main:app", host=host, port=port, reload=reload)
...
def model_select(
    role: str = typer.Argument(..., help="Role: general | coding | judge | embedding"),
    model_id: str = typer.Argument(...),
) -> None:
