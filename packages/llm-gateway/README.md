...
package namespace into `llm_gateway`.

> **Compatibility:** the old `agentos_gateway` import name is retained as a
> deprecated alias. Importing from `agentos_gateway` will raise a
> `DeprecationWarning` and resolve to the identical `llm_gateway` objects.
> New code should use `llm_gateway`.
...
cd llm-gateway
uv sync
uv run ruff check llm_gateway tests
...
(`general`, `coding`, `conversation`, `judge`). `allow_cloud`
defaults to `false`; in VS1 no cloud model is enabled, so cloud is unreachable.
Embedding is not exposed as a role alias until a supported embedding model is
registered.
...
`LLM_GATEWAY_AGY_CLIENT_ID` and `LLM_GATEWAY_AGY_CLIENT_SECRET` when exercising that
refresh path.
