...
from llm_gateway.registry import ModelRegistry, ActiveRoleManager, ModelEntry, RegistryError
...
class TestActiveRoleManager:
    ...
    def test_default_active_unpins_roles_for_node_agnostic_routing(self):
        ...
        assert "embedding" not in mgr.all()

    def test_stale_embedding_active_entry_is_hidden(self, tmp_registry):
        _, active_path, _, _ = tmp_registry
    ...
    def test_set_rejects_unsupported_role(self, tmp_registry):
        _, active_path, _, _ = tmp_registry
        mgr = ActiveRoleManager(active_path=active_path)

        with pytest.raises(ValueError):
            mgr.set("embedding", "old-embedding-model")
