...
- **Carried trees** — `src/migrate/`, `agents/<project>/`, and
  ...
  leaves `src/migrate` by reimplementation against the sdk rather than by
...
Carried component ownership, frozen under `src/migrate` and mined by
reimplementation:
