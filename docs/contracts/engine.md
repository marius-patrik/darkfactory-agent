...
> **Implementation:** `packages/inference/python-agent/agent/engines/contract.py`

Every inference engine sits behind a single Python Protocol.  inferctl
(`packages/inference/services/inferctl`, Go) is the controller; the Python contract is the seam
...
`packages/inference/python-agent/agent/engines/contract.py` implementation and
...
`packages/inference/python-agent/agent/engines/contract.py` and use Python >=
...
1. Create `packages/inference/python-agent/agent/engines/dynamo.py`, `class DynamoAdapter`.
...
*This document tracks `packages/inference/python-agent/agent/engines/contract.py`. It must be updated
