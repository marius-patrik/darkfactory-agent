...
4. Verify the returned JSON has `ok=true`, a canonical record ID, a snapshot,
   ...
   `agents memory status --json` is validated by its real contract: agent ID,
   non-negative record/event counts, and a SHA-256 projection hash; it does not
   expose a top-level `ok` field.
