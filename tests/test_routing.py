...
class TestResolveModel:
    ...
    def test_resolve_embedding_role_raises_when_no_model_registered(self, router_fixture):
        """Requesting role 'embedding' raises RoutingError when the pool is empty.

        VS1 defers the embedding model to VS2 (docs/gateway.md §"Deliberately
        left for VS2"; §13 adaptation point #5). The role is declared in
        ROLE_NAMES and in active.yaml but no model is seeded. This test asserts
        that the gateway rejects such requests with a clear RoutingError rather
        than silently returning None or routing to an unrelated model.
        """
        router, _, _ = router_fixture
        # The test fixture registry has no embedding-role model.
...
class TestGatewayMetadata:
    async def test_non_streaming_response_and_trace_include_gateway_metadata(self, router_fixture):
        ...
        metadata = result["agentos_gateway"]
...
class MockAsyncClient:
    """Mock httpx.AsyncClient for HTTP/NVCF routing tests."""
