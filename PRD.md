...
- [x] **L2 Review**: provider-agnostic DarkFactory Autoreview performs medium/Kimi review-to-clean, high/Sol final confirmation, issue review, bounded hash-bound autofix, and durable per-round receipts through canonical Agent OS. Merge is blocked until the exact check is green.
...
- [x] **L7 Release convergence**: one marker-owned, resumable engine classifies main/dev ancestry, reconciles divergence through reviewed heads, releases verified dev through `release/<sha>`, proves current Validate plus high-confirmed Autoreview evidence, verifies declared tag/artifact/publication policy, and emits downstream receipts without force, bypass, or default-branch writes. Main-ahead and missing-dev recovery fail closed behind an App-owned owner-contract issue because normal PR landing cannot simultaneously guarantee identical main/dev commit SHAs and prohibit every protected-ref write; the selected invariant must be recorded before either recovery path is enabled.
...
- [x] **M6 — Automated release lane**: `df release` status/plan/reconcile/run/verify plus the trusted scheduled/event workflow own dev-to-main convergence and post-release evidence. Acceptance: a green dev-ahead repository reaches one verified released main, linked issue closure, policy-branch preservation, and a durable downstream receipt without terminal intervention; post-release dev convergence uses the explicit owner-selected invariant, while the engine remains fail-closed before that selection.
