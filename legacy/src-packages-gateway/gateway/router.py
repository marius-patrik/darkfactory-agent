...
from agents.packages.gateway.gateway.registry import ModelEntry, ModelRegistry, ActiveRoleManager, generate_request_id
from agents.packages.gateway.gateway.trace import TraceLogger

ROLE_ALIASES = ("general", "coding", "judge", "embedding")
_BACKEND_META_KEY = "_agents_gateway_backend"
...
class Router:
    ...
    def resolve_model(self, model_id: str, allow_cloud: bool = False) -> ModelEntry:
        ...
        Supports:
        - Direct model IDs
        - Role aliases: 'general', 'coding', 'judge', 'embedding'
        """
    ...
    def _resolve_role(self, role: str) -> ModelEntry | None:
        ...
        if not pool:
            return self.registry.get(active_id) if active_id else None
    ...
    def _normalize_response(
        ...
        if not isinstance(choices, list):
            if gateway_metadata is not None:
                result["agents_gateway"] = gateway_metadata
            return result
