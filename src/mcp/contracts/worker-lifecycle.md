...
package at `src/server/inference/python-agent/agent/conductor/lifecycle.py` as
...
(`src/server/inference/python-agent/agent/conductor/`) to a standalone Go service,
...
| Path | Content |
|---|---|
| `src/server/inference/python-agent/agent/conductor/__init__.py` | Package init; re-exports all public names. |
| `src/server/inference/python-agent/agent/conductor/lifecycle.py` | Typed contract: all types + `WorkerLifecycle` Protocol. |
| `src/mcp/contracts/worker-lifecycle.md` | This document. |

Implementation files (VS0 stub, later Go adapter) live under
`src/server/inference/python-agent/agent/conductor/` and are **not** part of this
contract repo.
