...
export function defaultDraftPath(agentsHome: string, repository: string): string {
  if (!agentsHome || !path.isAbsolute(agentsHome)) throw new Error("A valid absolute ANDROMEDA_HOME is required for local issue draft state");
...
export async function createIssueDraft(
  ...
  try {
    const turn = await (runtime.executeDraftTurn ?? executeModelTurn)({
      ...
      environment: { ...runtime.environment, ANDROMEDA_HOME: runtime.agentsHome }
    }, {
