...
test("readManagedFiles supplies every required package-managed payload", async () => {
  ...
  try {
    ...
    if (previousAgentsHome === undefined) delete process.env.ANDROMEDA_HOME;
    else process.env.ANDROMEDA_HOME = previousAgentsHome;
...
test("readManagedFiles selects canonical Andromeda-data while allowing unrelated registered data repositories", async () => {
  ...
  try {
    const { registryPath } = await seedCanonicalManagedSource(root);
    process.env.ANDROMEDA_HOME = root;
    process.env.ANDROMEDA_ROOT = join(root, "distribution-root");
    process.env.ANDROMEDA_DATA_REPOS = registryPath;
    ...
    if (previousAgentsRoot === undefined) delete process.env.ANDROMEDA_ROOT;
    else process.env.ANDROMEDA_ROOT = previousAgentsRoot;
    await rm(root, { recursive: true, force: true });
  }
});
