...
import { enableEventSync, exportEventBundle, importEventBundle } from "../../cli/event-sync";
import { writeSecret } from "../../cli/secrets";
import { ensureSharedState, sharedStateAt, type SharedState } from "../../cli/state";
...
describe("operator entrypoint", () => {
  test("runs status against only the explicitly rooted disposable state", async () => {
    const { root, state } = await fixture();
    try {
      const child = Bun.spawn(["bun", path.resolve(import.meta.dir, "../cli.ts"), "status"], {
        cwd: root,
        env: {
          ...process.env,
          ANDROMEDA_ROOT: root,
