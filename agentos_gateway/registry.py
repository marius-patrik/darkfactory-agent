...
VS1-minimal: the registry is still a local-file ``save()``. Cluster-sync over
statesync/NATS (design §01 G4) lands in VS2 — see docs/gateway.md.
"""
...
# The role aliases a request may target (resolved by Router._resolve_role).
# VS1 adds "conversation" (the standing-brain voice role, §4.11/§4.5).
ROLE_NAMES = ("general", "coding", "conversation", "judge", "embedding")
...
def generate_request_id() -> str:
    return f"rommie-req-{uuid.uuid4().hex[:12]}"
