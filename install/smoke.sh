...
# checkout's `packages/core/src/manager/cli.ts`, then runs fast CLI commands
# (`agents state init` and `agents list`).

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
EXPECTED_SCRIPT="$ROOT_DIR/packages/core/src/manager/cli.ts"
...
if [ -L "$AGENTS_PATH" ]; then
  ...
  if [ -n "$GLOBAL_PKG" ]; then
    RESOLVED="$GLOBAL_PKG/packages/core/src/manager/cli.ts"
  fi
fi
